...
// ResampleStream. Built twice (see CMakeLists.txt) with PREFER_FIXEDPOINT=0
// and PREFER_FIXEDPOINT=1 from this same source; the outputs are diffed
// externally to confirm the fixed-point path stays within a few LSBs of the
// float reference.
...
void setup(void) {
  ...
  rcfg.copyFrom(info);
  // non-integer step, exercises fractional interpolation in both directions
  rcfg.step_size = 0.68f;
  resample.begin(rcfg);

  csv.begin(info);

  resample.write((const uint8_t *)arsineC256, sizeof(arsineC256));
  resample.flush();
}
