...
#define DEC_FACTOR 96                           // Decimating

#define NUM_CHANNELS 1                          // Only 1 and 2 channels work, results from 3 amd more are returned unordered
#define ADC_ATTENUATION ADC_ATTEN_DB_11         // ADC attenuation, see below
...
// returned
//        0 48 49 49 95 95 144 240  NA
//        1 48 47 NA NA NA  NA  NA  NA
//        2 48 NA NA NA NA  NA  NA  NA
//        3 48 48 48 96 96 144  NA  NA
//        4 47 48 47 96 NA  NA  NA  NA
//        5 47 48 48 NA NA  NA  NA  NA
//        6 49 48 47 97 97  NA  NA  NA
//        7 49 48 49 96 96 144 240 480
// ordered?  U  U  U  U  U   U   O   O

// It looks like maximum buffer size into stream copy is 1024
// If we specify larger buffer size it defaults to 1024 and the converters no longer work 
//   as buffer size needs to be divisible by num channels
#define BUFFER_SIZE_FACTOR (1024 / (NUM_CHANNELS*DEC_FACTOR*BIT_DEPTH/8))
#define ADC_BUFFER_SIZE (NUM_CHANNELS*DEC_FACTOR*BUFFER_SIZE_FACTOR) // Total number of samples (not bytes), needs to be divisible by 4
...
void setup(void) {
  ...
  adcConfig.buffer_size = ADC_BUFFER_SIZE;                      // in tital number of samples
