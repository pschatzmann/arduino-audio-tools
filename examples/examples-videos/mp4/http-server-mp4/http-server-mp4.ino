 ...
 * board), muxes the result into a fragmented (video-only) MP4 stream with
 * MuxerMP4, and publishes it via a plain AudioServer (WiFi) HTTP server so
 * it can be watched live with e.g. VLC or ffplay:
 * `vlc http://<esp32-ip-address>/`
 *
 * Companion client: http-client-mp4.ino (this project's DemuxerMP4 HTTP
 * client) - point its video_url at this sketch's IP to test the pair
 * end-to-end. This is the MP4 analog of http-server-avi-h264.ino (see that
 * file for the same pipeline built on AVI instead - the two are otherwise
 * line-for-line the same shape); MuxerMP4 writes a *fragmented* MP4
 * ('moof'+'mdat' per frame, no seeking/buffering needed), the same
 * streaming-friendly design MuxerAVI uses for AVI.
 *
 ...
 * CameraEncoderUDP example) if you don't need MuxerMP4/HTTP.
 *
...
void sendVideo(Print *out) {
  ...
  if (!muxer.begin()) {
    Serial.println("MuxerMP4 begin failed");
    return;
  }

  h264Encoder.setOutput(muxer);  // encoded NAL units go straight into the
                                 // 'video/mp4' fragments as they're produced
...
void setup() {
  ...
  // serves the result of sendVideo() as "video/mp4" to each connecting
  // client; AudioServer takes care of WiFi connection, HTTP handshake and
  // chunked transfer encoding
  server.begin(sendVideo, "video/mp4");
}
