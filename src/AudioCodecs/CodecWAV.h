...
class WAVDecoder : public AudioDecoder {
 public:
  ...
  void setOutput(Print &out_stream) { this->out = &out_stream; }
...
class WAVEncoder : public AudioEncoder {
 public:
  ...
  WAVEncoder() { audioInfo = defaultConfig(); }
  ...
  void setOutput(Print &out) { stream_ptr = &out; }
  ...
  virtual void setAudioInfo(WAVAudioInfo ai) {
    ...
    audioInfo.byte_rate =
        audioInfo.sample_rate * audioInfo.bits_per_sample * audioInfo.channels;
  ...
  virtual void begin() { begin(audioInfo); }
  ...
  void setDataOffset(uint16_t offset) { this->offset = offset; }
 ...
 protected:
  ...
  void writeFMT(Print *stream_ptr) {
    uint16_t fmt_len = 16;
    uint32_t byteRate = audioInfo.sample_rate * audioInfo.bits_per_sample *
                        audioInfo.channels / 8;
