...
class PWMDriverSTM32 : public DriverPWMBase {
    ...
    struct PWMPin {
        ...
        PWMPin(HardwareTimer *p_timer, int channel, int pin, int maxValue, int pwmFrequency=30000){
        ...
        void begin(){
            TRACEI();
            p_timer->setPWM(channel, pin, pwm_frequency, 50); // 30k Hertz, 50% dutycycle
            active = true;
        }
        ...
        void setRate(int rate){
            if (active){
                ...
                p_timer->setCaptureCompare(channel, sample, PERCENT_COMPARE_FORMAT); // 50%
            }
        }
    };
    ...
    class PWM {
        public:
            ...
            bool addPin(int pin){
                LOGI("addPin: %d", pin);
                TIM_TypeDef *p_instance = (TIM_TypeDef *)pinmap_peripheral(digitalPinToPinName(pin), PinMap_PWM);
                ...
                if (p_timer->getHandle()->Instance!=p_instance){
                    LOGE("Invalid pin %d with timer %s for timer %s",pin, getTimerStr(p_instance), getTimerStr(p_timer->getHandle()->Instance));
                    return false;
                }
        ...
        protected:
            ...
            const char* getTimerStr(TIM_TypeDef* inst){
                if (inst==TIM1) return "TIM1";
                else if (inst==TIM2) return "TIM2";
                else if (inst==TIM3) return "TIM3";
                else if (inst==TIM4) return "TIM4";
                else if (inst==TIM5) return "TIM5";
                return "N/A";
            }
    };
