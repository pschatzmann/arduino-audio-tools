...
class EqualizerNBands : public ModifyingStream {
 public:
  ...
  /// @param volume Volume level (0.0 to 1.0) mapped to -12dB to +12dB
  bool setBandGain(int band, float volume) {
    ...
    vol = map<float>(vol, 0.0f, 1.0f, -12.0f,
                     12.0f);  // Map 0.0-1.0 to -12dB to +12dB
  ...
  /// Get current gain for a specific band as normalized volume (0.0 to 1.0)
  float getBandGain(int band) {
    if (band < 0 || band >= NUM_BANDS) return 0.0f;
    return map<float>(pendingGains[band], -12.0f, 12.0f, 0.0f, 1.0f);
  }
  ...
  class EQFIRFilter : public Filter<SampleT> {
   public:
    ...
    SampleT process(SampleT sample) override {
      if (!activeKernel) return sample;  // Pass-through if no kernel set
   ...
   protected:
    ...
    int idxHist = 0;
    volatile int16_t* activeKernel;  // Pointer to active kernel
