...
class Task {
 public:
  Task(const char *name, int stackSize, int priority = 1, int core = -1) {
    (void)name; (void)stackSize; (void)priority; (void)core;  // parameters unused
  }
  Task() = default;
  ~Task() { remove(); }
  ...
  bool begin(std::function<void()> process) {
    ...
    loop_code = process; terminate_flag = false; paused = true;  // start suspended similar to FreeRTOS create+suspend pattern
  ...
  void remove() {
    terminate_flag = true; resume();  // wake if paused
    if (running_thread.joinable()) running_thread.join();
  }

  void suspend() {
    std::lock_guard<std::mutex> lk(mtx); paused = true;
  }

  void resume() {
    { std::lock_guard<std::mutex> lk(mtx); paused = false; } cv.notify_all();
  }
 ...
 protected:
  ...
  void thread_loop() {
    ...
    while(!terminate_flag.load()) {
      ...
      if (loop_code) loop_code(); else nop();
    }
  }
};
