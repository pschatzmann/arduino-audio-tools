...
/**
 * @brief Typed Stream Copy which supports the conversion from channel to 2
 * channels. We make sure that we allways copy full samples
 * @ingroup tools
 * @tparam T
...
class StreamCopyT {
 public:
  ...
  /// Provides a pointer to the copy source. Can be used to check if the source
  /// is defined.
  Stream *getFrom() { return from; }

  /// Provides a pointer to the copy target. Can be used to check if the target
  /// is defined.
  Print *getTo() { return to; }

  /// copies the data from the source to the destination and returns the
  /// processed number of bytes
  inline size_t copy() {
    p_converter = nullptr;
    return copyBytes(buffer_size);
  }

  /// copies the data from the source to the destination and applies the
  /// converter - the result is the processed number of bytes
  inline size_t copy(BaseConverter &converter) {
    p_converter = &converter;
    return copyBytes(buffer_size);
  }

  /// copies the inicated number of bytes from the source to the destination and
  /// returns the processed number of bytes
  ...
  /// Copies audio for the indicated number of milliseconds: note that the
  /// resolution is determined by the buffer_size
  ...
  /// Activates the check that we copy only if available for write returns a
  /// value
  ...
  /// Defines the minimum frame size that is used to round the copy size: 0 will
  /// automatically try to determine the value
  void setMinCopySize(int size) { min_copy_size = size; }

  /// Activate the synchronization from the AudioInfo form the source to the
  /// target
