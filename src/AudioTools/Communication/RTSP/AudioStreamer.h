 ...
 * 
 * The AudioStreamer class is responsible for converting audio data from an IAudioSource
 * into RTP (Real-time Transport Protocol) packets and streaming them over UDP to RTSP clients.
 * This class handles the complete RTP streaming pipeline including:
 * 
 ...
 * - Periodic streaming using ESP32 timers
 ...
 * 
 * @note This class requires an ESP32 platform for timer functionality
 * @author Thomas Pfitzinger
 * @version 0.1.1
 */
class AudioStreamer {
 ...
 public:
   ...
   * Initializes internal state including ports, sequence numbers, and timer configuration.
   * No audio source is assigned - use setAudioSource() to configure streaming source.
   * 
   ...
   * 
   * Creates a new AudioStreamer instance and immediately configures it with the specified
   * audio source. This is equivalent to calling the default constructor followed by
   * setAudioSource().
   * 
   * @param source Pointer to an object implementing the IAudioSource interface.
   *               The source provides audio data and format information for streaming.
   * @note The audio source object must remain valid for the lifetime of the AudioStreamer
   * @see IAudioSource
   */
   ...
   * This method automatically initializes the audio source configuration including
   * fragment size and timer period based on the source's format specifications.
   * 
   * @param source Pointer to an IAudioSource implementation. Must not be nullptr.
   ...
   * 
   * Extracts and caches configuration parameters from the audio source including
   * fragment size (bytes per packet) and timer period (microseconds between packets).
   * This method must be called before streaming can begin.
   * 
   * @return true if initialization succeeded, false if audio source or format is invalid
   * @note Fragment size determines how much audio data is sent per RTP packet
   * @note Timer period controls the streaming rate and must match the audio sample rate
   */
   ...
   * @return true if transport initialization succeeded, false on socket creation failure
   * 
   * @note Port allocation: RTP uses even ports (6970, 6972, ...), RTCP uses odd ports (6971, 6973, ...)
   ...
   * 
   * Constructs and sends one RTP packet containing audio data from the configured source.
   * This method handles the complete RTP packet creation process including:
   ...
   * 
   * @return Number of bytes sent in the packet payload, or negative value on error
   * @retval -1 RTP buffer not allocated or audio source not configured
   * 
   * @note This method is typically called periodically by the timer-driven streaming
  ...
  int sendRtpPacketDirect() {
    ...
    udpsocketsend(m_RtpSocket, mRtpBuf.data(), HEADER_SIZE + bytesRead1, m_ClientIP,
                  m_ClientPort);
   ...
   * - Creating and configuring the ESP32 timer for periodic streaming
   ...
   * - Stopping the ESP32 timer to cease packet transmission
   * 
   * @note Does not release UDP transport or buffers - use releaseUdpTransport() separately
 ...
 protected:
   ...
   * 
   * This static method is called periodically by the ESP32 timer to send RTP packets.
   * It handles timing measurements and calls sendRtpPacketDirect() to transmit audio data.
   * Performance monitoring ensures the streaming keeps up with real-time requirements.
   * 
   * @param audioStreamerObj Void pointer to the AudioStreamer instance (cast from timer arg)
   * @note This is a static method suitable for use as an ESP32 timer callback
  ...
  static void doRtpStream(void *audioStreamerObj) {
    ...
    if (stop - start > 20000) {
      log_w("RTP Stream can't keep up (took %i us, 20000 is max)!",
            stop - start);
    }
  }
};
