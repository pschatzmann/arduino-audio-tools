 ...
 * by reading from an input stream and writing decoded PCM data to an output stream.
 * Unlike AudioDecoder which uses a write-based interface, StreamingDecoder uses
 * a pull-based approach where you call copy() to process data.
 * 
 * @note This is more efficient than the write-based AudioDecoder interface
 * for streaming scenarios where you have direct access to input and output streams.
 * 
 * @ingroup codecs
...
class StreamingDecoder : public AudioInfoSource {
 public:
   ...
   * 
   * This is more efficient than feeding the decoder with write: just call copy() 
   * in the loop to process data from the input stream.
   * 
   ...
   * 
   * @return C-string containing the MIME type (e.g., "audio/mpeg", "audio/flac")
   */
...
class StreamingDecoderAdapter : public StreamingDecoder {
 public:
   ...
   * @param copySize Buffer size for data transfer (default: DEFAULT_BUFFER_SIZE)
   */
 ...
 * 
 * This class automatically detects the audio format from incoming streaming data
 * and selects the appropriate decoder from a collection of registered decoders.
 * The format detection is performed using the MimeDetector on the first chunk
 * of data, and the detected data is preserved for the selected decoder using
 * a buffered stream approach.
 * 
 * Key features:
...
class MultiStreamingDecoder : public StreamingDecoder {
 ...
 protected:
  /**
   * @brief Simple buffered stream that prefixes buffered data before reading from the main stream
   * 
   * This internal helper class creates a virtual stream that first serves data from
   * a buffer (used for format detection), then seamlessly continues reading from
   * the main input stream. This ensures that data used for MIME detection is not
   * lost and is available to the selected decoder.
   */
 ...
 * it allows you to use a StreamingDecoder with the write-based AudioDecoder API.
 * It uses a ring buffer and queue to convert write() calls into a stream that
 * the StreamingDecoder can read from.
 * 
