...
struct USBAudioConfig : public AudioInfo {
    ...
    size_t slab_size_bytes = 1024;  ///< total slab memory; actual size rounds up to block boundary
...
public:
    ...
    USBAudioConfig defaultConfig(RxTxMode mode = TX_MODE) {
        ...
        cfg.slab_size_bytes = 1024;  ///< total slab memory; actual size rounds up to block boundary
    ...
    bool begin(USBAudioConfig cfg) {
        if (!cfg) {
            LOG_ERR("USBAudioStream: invalid AudioInfo "
                    "(rate=%u ch=%u bits=%u)",
        ...
        if (k_mem_slab_init(&slab_, slab_buf_.data(),
                            block_sz, slab_block_count) != 0) {
    ...
    int availableForWrite() override { return static_cast<int>(ring_.availableForWrite()); }
...
protected:
    ...
    void _sendFromRing() {
        ...
        while (ring_.available() >= block_sz) {
            ...
            int ret = usbd_uac2_send(uac2_dev_,
                                     cfg_.terminal_id,
                                     buf,
    ...
    static void s_terminal_update(const struct device *dev,
                                  ...
                                  void *user_data)
    {
        ARG_UNUSED(dev); ARG_UNUSED(terminal); ARG_UNUSED(microframes);
    ...
    static void *s_get_recv_buf(const struct device *dev,
                                ...
                                void *user_data)
    {
        ARG_UNUSED(dev); ARG_UNUSED(terminal);
        auto *self = static_cast<USBAudioStream *>(user_data);

        if (!self->is_active_ || self->cfg_.rx_tx_mode != RX_MODE)
            return nullptr;
     ...
     */
    static void s_data_recv(const struct device *dev,
                            uint8_t terminal,
                            void *buf,
                            uint16_t size,
                            void *user_data)
    {
        ...
        size_t written = 0;
        if (self->p_output) {
            written = self->p_output.write(
                static_cast<const uint8_t *>(buf), size);

        } else {
            written = self->ring_.writeArray(
                static_cast<const uint8_t *>(buf), size);
     ...
     */
    static void s_buf_release(const struct device *dev,
                              uint8_t terminal,
                              void *buf,
                              void *user_data)
    {
