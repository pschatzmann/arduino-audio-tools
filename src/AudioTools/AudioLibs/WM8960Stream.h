...
class WM8960Config : public I2SConfig {
 public:
  ...
  // optional features: use bitmask with
  // WM8960_FEATURE_MICROPHONE,WM8960_FEATURE_HEADPHONE,WM8960_FEATURE_SPEAKER
  int8_t features = -1;
};
 ...
 * @copyright GPLv3
 *
 * @note Supported only on Arduino platforms with the WM8960 library. For a
 * Platform independent implementation use the I2SCodecStream with the
...
class WM8960Stream : public AudioStream {
 public:
  ...
  void setVolumeIn(float vol) { adjustInputVolume(vol); }

  void setVolumeOut(float vol) { setOutputVolume(vol); }

  /// provides the volume
  float volumeIn() { return volume_in; }

  float volumeOut() { return volume_out; }
 ...
 protected:
  ...
  void setOutputVolume(float vol) {
    ...
    int vol_int =
        volume_out == 0.0 ? 0 : map(volume_out * 100, 0, 100, 30, 0x7F);
    ...
    if (cfg.features == -1) {
      switch (mode) {
        ...
        case TX_MODE:
          ...
          break;
        case RXTX_MODE:
          cfg.features = WM8960_FEATURE_MICROPHONE1 | WM8960_FEATURE_HEADPHONE |
  ...
  bool configure_clocking() {
    ...
    if (!mtb_wm8960_configure_clocking(
            cfg.vs1053_mclk_hz, cfg.vs1053_enable_pll,
            sampleRate(cfg.sample_rate), wordLength(cfg.bits_per_sample),
  ...
  void volumeError(float vol) { LOGE("Invalid volume %f", vol); }
};
