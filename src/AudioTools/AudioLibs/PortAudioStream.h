#pragma once
/**
 * @brief Generic Implementation of sound input and output for desktop
 * environments using portaudio
 *
 */
 ...
 * @ingroup io
 *
 * @note Supported only on platforms supported by the PortAudio library:
 * windows, linux, macos, etc. but not on microcontrollers like Arduino or
 * ESP32.
 */
class PortAudioStream : public AudioStream {
 public:
  PortAudioStream() { TRACED(); }
  ...
  bool begin() override { return begin(defaultConfig()); }
  ...
  operator bool() override { return err == paNoError; }
 ...
 protected:
  ...
  int bytesPerSample() {
    ...
    return info.bits_per_sample == 24 ? sizeof(int24_t)
