...
public:
  ...
  /// skipped
  void setStartTime(long startSeconds) { start_time = startSeconds; }

  /// Defines (an optional) the end time in seconds. After the end time no audio
  /// is played and available() will return 0
  void setEndTime(long endSeconds) { end_time = endSeconds; }
  ...
  void setAudioInfo(AudioInfo info) override { p_info->setAudioInfo(info); }
...
protected:
  ...
  void calculateTime(int bytes) {
    ...
    current_time =
        (static_cast<double>(bytes) / bytes_per_second) * compression_ratio;
  }
};
