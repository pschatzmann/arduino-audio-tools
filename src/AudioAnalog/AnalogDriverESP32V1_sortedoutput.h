...
public:
  ...
  int available() override { return active_rx ? cfg.conv_frame_size : 0; }
...
protected:
  ...
  public:
    ...
    // provides int16_t data from the Analog to Digital Converter
    // ----------------------------------------------------------
    size_t readBytes(uint8_t *dest, size_t size_bytes) override {
      TRACED();
      ...
      memset(&result_data, 0, sizeof(result_data)); // init with 0
      uint32_t bytes_read;
      if (adc_continuous_read(self->adc_handle, 
                              (uint8_t *)result_data,
                              sizeof(result_data), 
                              &bytes_read,
                              self->cfg.timeout) == ESP_OK) {
        ...
        LOGD("adc_continuous_read -> %u bytes / %d samples", (unsigned) bytes_read, samples_read);
